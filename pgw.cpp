...
int main()
{
    ...
    rocksdb::Status status = rocksdb::DB::Open(options, "./pgwdb", &db);
    assert(status.ok());
...
void *handlediam(void *diam){
   ...
   //         std::cout<<"avp sessid : "<<sessidval<<std::endl;
...
void *sub(void *args){
    ...
    char* host="test_host";
    const char* hostid=std::to_string(arg.arg1).c_str();
    char h[strlen(host)+strlen(hostid)];
    strcpy(h,host); // copy string one into the result.
    strcat(h,hostid); // append string two to the result.
    //const char* sid=std::to_string(subsid).c_str();
...
void *connection_handler(void *threadid)
{
    ...
    serv_addr.sin_addr.s_addr = inet_addr("10.195.84.157");
